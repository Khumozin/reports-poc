...
@Component({
  ...
  styleUrl: './year-page.component.scss',
...
export class YearPageComponent implements OnInit {
  ...
  onGetData(): void {
    const year = moment(this.form.getRawValue().date).format('yyyy');
    const MM = moment(this.form.getRawValue().date)
      .format('yyyy-MM')
      ?.substring(5);

    const months = this.getPreviousMonths(MM, year)
    ...
    this.data$
      .pipe(
        map((data) =>
          data.filter((d) => months.includes(d.Date.substring(0, 7)) && d.Dealer === dealer)
        ),
        map((data) => {
          return [
            {
  ...
  getPreviousMonths(currentMonth: string, year: string): string[] {
    return Array.from(
      { length: parseInt(currentMonth) },
      (_, index) => index + 1
    )
